...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        ...
        if ($type == 'verify') {
            ...
            $this->email->message('Kami ingin menginformasikan bahwa anda sudah berhasil melakukan proses registrasi akun<br>
                        <p>Silahkan menekan link yang ada di bawah pesan ini untuk mengaktifkan akun anda.
                        </p><br>Click this link to verify your account : <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Activate</a>
                        <br>
                        Terima kasih karena telah menggunakan Go Traveling sebagai alternatif anda dalam melakukan perjalanan wisata ke Kabupaten Bandung Barat. 
                        Salam,<br>
    ...
    public function forgotPassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $email = $this->input->post('email');
            $user = $this->db->get_where('user', ['email' => $email, 'flag_active' => 1])->row_array();
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('flash', 'Silahkan cek email anda untuk melanjutkan proses reset password!');
                redirect('auth/forgotpassword');
            } else {
                $this->session->set_flashdata('flash_error', 'Email Belum terdaftar atau aktif!');
                redirect('auth/forgotpassword');
            }
        }
    }
